...
├── openspec/                ← OpenSpec 規劃與規格變更骨架
├── scripts/
│   └── issue_lifecycle.sh   ← Issue 查重 / 更新 / 驗證後關閉 helper
├── AGENTS.md                ← AI 共用核心規則
...
├── issue-lifecycle-governance.md ← Issue / OpenSpec lifecycle SOP
...
7. 依 `issue-lifecycle-governance.md` 建立或重用 GitHub Issue，必要時建立 OpenSpec change

## 佈署與核可節奏（建議）

1. 工程師在 `ai/<工程師>/<任務>` 分支完成實作與本地驗證。
2. 非 trivial 工作先建立或重用 GitHub Issue；新能力、治理、安全、架構或模糊需求先建立 OpenSpec change。
3. push 後以 `https://<your-staging-domain>/p/<engineer>/<task>/` 進行分支預覽自我驗證。
   - 實際 domain / path / 必測端點 請寫在 `AGENTS.local.md`。
   - 正式 preview 應使用專案自有 host 或隔離路徑；不要把 `ngrok` / `localtunnel` / `localhost.run` 當成 merge gate 的正式預覽。
4. 每次修改完成回報（含中間交付）必須附上「合併前預覽網址」，且只需提供本次修改目標頁（若有指定單號/ID 需附完整路徑）。
5. 驗證項目至少包含登入流程、主頁進入、目標 API 回應與基本流程可操作。
6. 將驗證截圖與結果回報 PM，由 PM 給最終「可上線」判定。
7. 通過後提 PR 到 主線分支，並要求 CI 完全通過（含 `/api` 健檢與 healthcheck）。
8. PR 建立後每次更新 commit 都要確認 required checks 全綠；任一檢查非綠燈時，禁止宣告完成或請求合併。
...
9. 詢問是否關閉 Issue 前，AI 必須先完成驗證並附證據；使用者或 maintainer 明確確認後才關閉。
10. 以 PM 最終核准為前提完成 merge，並於 merge 後回報 staging 覆核結果。
